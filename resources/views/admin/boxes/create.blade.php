...
@section('title', 'Créer une Box - LynBox')
...
<div class="space-y-8">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-3xl font-bold">Créer une nouvelle Box</h1>
    </div>

    <div class="glass p-8 rounded-2xl max-w-2xl">
        <form action="{{ route('admin.boxes.store') }}" method="POST" class="space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-semibold mb-2">Titre de la Box *</label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="Ex: Summer Glow Beauty Box"
            ...
            <div>
                ...
                <textarea name="description" placeholder="Décrivez votre box en détail..." rows="4"
                    class="w-full bg-white/5 border {{ $errors->has('description') ? 'border-red-500' : 'border-white/10' }} rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:border-indigo-500/50 focus:outline-none transition resize-none" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="price" value="{{ old('price') }}" placeholder="29.99" step="0.01" min="0"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 100) }}" min="0"
                ...
                <div>
                    ...
                    <select name="billing_cycle" class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-indigo-500/50 focus:outline-none transition" required>
                        ...
                        <option value="quarterly" {{ old('billing_cycle') == 'quarterly' ? 'selected' : '' }}>📅 Trimestriel</option>
            ...
            <div>
                <label class="flex items-center gap-3 p-3 rounded-lg hover:bg-white/5 cursor-pointer transition">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-5 h-5 rounded accent-indigo-500">
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-semibold transition">
                    <i class="fas fa-save mr-2"></i> Créer la Box
                </button>
